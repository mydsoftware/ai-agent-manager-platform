...
export async function streamAgent({ agent, tenantId, userId, input, maxIterations = 5, memoryLimit = 8 }: StreamAgentInput, emit: (event: any) => void) {
  ...
  for (let iteration = 1; iteration <= Math.min(Math.max(maxIterations, 1), 10); iteration++) {
    ...
    if (decision.final !== undefined) { emit({ type: 'delta', text: String(decision.final) }); emit({ type: 'done', iteration }); return }
