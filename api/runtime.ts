...
export async function runAgent(input: AgentRuntimeInput) {
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      return tx.agentRun.update({ where: { id: run.id }, data: { status: 'COMPLETED', output: { text: result.text, trace: result.trace }, tokensUsed: result.tokensUsed, creditsUsed, iterations: result.iterations, finishedAt: new Date() } })
    })
  } catch (error: any) {
