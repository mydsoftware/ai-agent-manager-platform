...
function parseDecision(text: string): Decision {
  try { const parsed = JSON.parse(text); if (parsed?.tool?.name) return parsed as Decision; if (typeof parsed?.final === 'string') return parsed as Decision } catch {}
  return { final: text }
}
...
export async function executeAgentLoop({ agent, tenantId, userId, input, maxIterations = 5 }: LoopInput) {
  ...
  for (let iteration = 1; iteration <= Math.min(Math.max(maxIterations, 1), 10); iteration++) {
    ...
    const response = await provider.generate({ system: `${agent.systemPrompt || `You are ${agent.name}.`}\n${toolInstructions}`, prompt: context, temperature: 0.1, maxTokens: 1200 })
    totalInput += response.inputTokens || 0; totalOutput += response.outputTokens || 0
